...
function mapApiPublication(row, index) {
  return {
    img:
      row.image_url ||
      row.image ||
      "/Lagos Unveils Investment Deal Book, Bold Vision for a 21st-Century Economy.png",
    tag: row.tag || "Report",
...
function PublicationsHero() {
  return (
    <section
      ...
      style={{
        backgroundImage: "url(/publication-bg.png)",
    ...
    >
      <div className="relative mx-auto max-w-310 font-inter">
        ...
        <p className="mt-5 max-w-2xl text-sm leading-7 text-gray-200 sm:text-base">
          Browse reports, investment guides, policy papers, research
          publications and strategic documents that provide deeper insights into
          Lagos' economy and investment opportunities.
        </p>
      </div>
    </section>
  );
...
function PublicationsFilterBar({
  ...
  return (
    <div className="bg-green400 px-4 md:px-10 lg:px-20 py-10">
      <div className="mx-auto max-w-310">
        <div className="flex flex-col items-stretch gap-3 p-2 sm:flex-row sm:items-center sm:justify-end">
          <div className="flex flex-1 max-w-100 items-center gap-3 rounded-full border-white border px-5 py-2.5">
            ...
            <input
              ...
              className="flex-1 bg-transparent text-sm text-white placeholder:text-white/90 focus:outline-none"
            />
          </div>
          <div className="relative flex items-center rounded-md bg-white/95 px-5 py-2.5 text-sm text-slate-600 sm:w-44">
            <select
              value={category}
          ...
          <div className="relative flex items-center rounded-md bg-white/95 px-5 py-2.5 text-sm text-slate-600 sm:w-36">
            <select
              value={year}
...
function PublicationCard({ pub }) {
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-md bg-white shadow-[0_4px_12px_rgba(0,0,0,0.06)] il-card">
      <Link
        to={to}
        className="block aspect-4/3 w-full overflow-hidden il-card-media"
      >
        <img
          src={pub.img}
          alt={pub.title}
      ...
      <div className="flex flex-1 flex-col gap-3 px-6 pt-5 pb-6">
        <div className="flex items-center gap-3 text-[11px] font-medium text-slate-500">
          <span className="rounded-sm bg-slate-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-green100">
            {pub.tag}
          </span>
        ...
        <h3 className="text-base font-bold text-black100">
          <Link to={to} className="transition hover:text-green100">
            {pub.title}
          </Link>
        </h3>
...
function PublicationsGrid({ items, loading, error }) {
  return (
    <section className="bg-[#F7F8FA] px-4 md:px-10 lg:px-20 py-14 lg:py-20">
      <div className="mx-auto max-w-310">
        {loading && (
  ...
  return (
    <section className="dealbook-bg flex flex-col gap-6 px-4 py-10 md:flex-row md:items-center md:gap-10 md:px-10 md:py-14 lg:px-20">
      ...
      <div className="flex max-w-sm flex-col gap-6 md:flex-1 md:items-center">
        ...
        >
          <span className="font-jost text-xs font-bold text-white">
            Download
          </span>

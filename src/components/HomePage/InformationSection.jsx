  ...
  return (
    <section className="px-4 md:px-10 lg:px-20 py-20 md:py-30 border-t border-t-yellow bg-green100 flex flex-col gap-20">
        <div className="flex flex-col gap-10">
            <h3 className="text-white font-jost font-medium text-4xl relative after:absolute after:content-[''] after:left-0 after:h-2 after:w-25 after:bg-white after:-bottom-4">Reach out to our Information Center</h3>
            <p className="text-white font-jost text-xl font-medium max-w-[50ch]">Need help with travel arrangements, hotel bookings, or media accreditation? Our team is ready to assist you every step of the way.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-5">
            {
                informations.map(({icon: Icon, heading, title, sub, link}, i) => <div key={heading} className="p-10 flex flex-col gap-12 bg-black/10 rounded-4xl ">
                    <div className="flex flex-col gap-5">
                        <div className="w-16 h-16 flex items-center justify-center border border-white/30 bg-black/10 rounded-3xl">
                        ...
                        <div className="flex flex-col gap-2">
                            <h4 className="text-white text-xl font-medium font-jost">{title}</h4>
                            <p className="text-white text-base font-medium font-jost">{sub}</p>
                        </div>
                    </div>
